...
// 2. **第一铁律：有记忆的工程绝不隐藏、绝不剔除。** 白名单只用来清理「零节点的空壳
//    登记」（以用户主目录启动产生的临时登记、宿主已删除的历史目录留下的空记录）；
//    只要库里还有节点，无论 scope 是否在名单里都照常保留并展示 —— v0.6.3 曾按 scope
//    无条件剔除，配合 hash 规范化口径变更把存量工程连同真实记忆整棵删掉，这是
//    「看板下拉框空掉、记忆归零」的事故根因，本文件把修复后的契约钉死；
// 3. 方案 B 锚点约束：只有「宿主当前正在打开的合法工作区」（或本身有数据的工程）在
//    零记忆时才保留为 `工程名 [工作区名] (0)` 锚点；
// 4. GET /api/projects 的每条清单项带 workspaceName，且**同一目录的历史 hash 变体**
//    与**与工作区同名的存量工程**都被认作该工作区的归属（hash 容错与同名对齐）；
// 5. GET /api/nodes 对失效作用域不再 404，回空树 + 可读提示。
...
describe('dsh-plugin-tlmemory 按 DSH 工作区对齐工程', () => {
  ...
  it('GET /api/projects：名单外但有记忆的工程照常呈现并带上归属，零节点空壳被静默清理', async () => {
    const loaded = buildRegistry()
    const db = new MemoryDB(':memory:')
    // 名单外 + 有记忆（存量 hash 漂移工程，名字与工作区同名 ⇒ 同名对齐）
    db.registerProject('repo:1c5b8d3f3502', 'my-dsh-plugins', 'C:/Users/ZhuanZ/.dsh/profiles/web')
    db.upsertLeaf('repo:1c5b8d3f3502', ['DSH客户端插件'], '视图切换', '由 conversationStore.view 驱动', ['dsh'])
    // 名单外 + 零节点（纯空壳登记）
    db.registerProject(ORPHAN_SCOPE, 'ZhuanZ', 'C:/Users/ZhuanZ')
    ...
    // 第一铁律：有记忆的存量工程绝不被隐藏（这正是上一版的事故点）
    expect(scopes).toContain('repo:1c5b8d3f3502')
    expect(db.getNodesByScope('repo:1c5b8d3f3502')).toHaveLength(2)
    // 同名对齐：它虽不在 workspace.json 里，但归属被认到 my-dsh-plugins
    expect(body.data.find((item) => item.scope === 'repo:1c5b8d3f3502')?.workspaceName).toBe('my-dsh-plugins')
  ...
  it('有数据的工程即使被当作「当前工程」传入也如实上报（不再被锚点约束吞掉）', async () => {
    const loaded = buildRegistry()
    const db = new MemoryDB(':memory:')
    db.registerProject(ORPHAN_SCOPE, 'ZhuanZ', 'C:/Users/ZhuanZ')
    ...
    // 宿主确实在这个目录里 ⇒ 如实上报为当前工程，而不是回 null 让前端失去落点
