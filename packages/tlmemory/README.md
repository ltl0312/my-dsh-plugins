...
- **零记忆工程自动清理 / 工程名全局唯一**：没有任何记忆文件的工程会被自动删除（含遗留的空目录骨架），**仅豁免宿主当前活跃工程** —— 它零记忆时也常驻下拉框（显示为 `工程名 (0)`），选中即进入「暂无记忆，点击上方「+ 新建记忆」开始沉淀」的空状态，作为「当前工程就绪、可随时沉淀」的心智锚点；同名工程（不同路径的同名仓库、目录改名遗留的旧作用域）自动追加 `scope` 短标识收敛为唯一名，手工重命名撞名时明确报错。
