...
工程落地采用 Monorepo 单仓库多包管理模式，插件后端直接嵌入 DSH 宿主 Node.js 运行时，严格遵守 Cordis 微内核“注册即副作用”可逆注销原则与上下文声明合并规范；前端采用 Vue 3 构建轻量单页面应用，以后端内嵌的严格回环网络服务（127.0.0.1）为通信中枢，通过 Iframe 容器沙箱挂载于 DSH 侧边栏插槽，利用 WebSocket 建立毫秒级双向命中感知链路。为确保自主 AI-Agent 能够按计划无歧义执行，本规范拆解为五个严格递进的研发阶段，全链路贯彻输入净化、提示词定界符隔离与 CVE-2026-82533 回环穿透防御。
...
根据 DSH 架构规范，插件无需修改任何官方核心代码，直接通过在 `$DSH_HOME/cordis.patch.yml` 或特定 Profile 补丁中声明挂载：
...
| **原生依赖放行** | `better-sqlite3` 编译不被 pnpm 拦截            | 运行 `pnpm approve-builds better-sqlite3 esbuild`            |
